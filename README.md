...
- 渲染后端：`WebGpu`，优先使用高性能 GPU，`max_fps = 60`；光标缓动为 Constant，`animation_fps = 1` 以降低空闲功耗。
- 主题配色：`Gruvbox dark, medium (base16)`。
- 背景：使用纯色 `#161A22`，关闭透明和 Windows 背景特效以降低全屏/还原时的合成压力。
- 标签栏：自定义胶囊标签、进程图标、管理员标识、未读输出提示。
- 状态栏：右侧显示日期时间和电池状态。
- 快捷键：禁用默认键盘绑定；保留 WezTerm 默认鼠标绑定，以支持 Codex、Claude 等全屏终端程序。
...
如果迁移到新机器，Git Bash 路径要么保持一致，要么修改 `config/launch.lua` 里的 `git_bash`。`scripts/pwsh-wezterm.ps1` 里的 `gitbash` 别名在固定路径不存在时，会自动从 PATH 中 `git.exe` 的位置推导。
...
| `Alt+q` | 快速选取模式（抓取屏幕上的路径/URL/哈希） |
| `Alt+e` | 打开 emoji/符号选择器 |
| `Alt+Ctrl+/` | 垂直分屏 |
| `Alt+Ctrl+\` | 水平分屏 |
| `Alt+Ctrl+h/j/k/l` | 切换窗格 |
| `Alt+Ctrl+p` | 窗格选择模式（按数字跳转） |
| `Alt+y` | 输入并执行 `yazi` |
